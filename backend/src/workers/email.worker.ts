...
import { acquireRateLimit } from "../services/rate-limiter.service";
import { updateEmailIndex } from "../services/elasticsearch.service";
import { notifySlackHourlyLimitReached } from "../services/slack.service";
...
const worker = new Worker(
  ...
  async (job: Job, token?: string) => {
    ...
    if (email.status === "sending") {
      console.log(
        `Email ${emailId} is already being processed. Skipping.`
      );
    ...
    if (email.status === "failed") {
      console.log(
        `Email ${emailId} is marked failed. Skipping automatic resend.`
      );
    ...
    if (claimResult.rowCount === 0) {
      console.log(
        `Email ${emailId} could not be claimed. ` +
    ...
    console.log(
      `Email ${emailId} marked as sent in PostgreSQL.`
    );
